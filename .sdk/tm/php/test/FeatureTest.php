...
// CatherineSchulmanQuotes SDK feature test
//
...
class FtCtrl extends CatherineSchulmanQuotesControl
{
...
class FtClient
{
    ...
    private CatherineSchulmanQuotesUtility $utility;

    public function __construct(CatherineSchulmanQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): CatherineSchulmanQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CatherineSchulmanQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CatherineSchulmanQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CatherineSchulmanQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CatherineSchulmanQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(CatherineSchulmanQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CatherineSchulmanQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CatherineSchulmanQuotesResponse($fetched);
        }
    ...
    private function fail(CatherineSchulmanQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CatherineSchulmanQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
