...
// CatherineSchulmanQuotes SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CatherineSchulmanQuotesOmni::makeStructRunner(
    ...
    {
        return CatherineSchulmanQuotesOmni::objmodel(CatherineSchulmanQuotesOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return CatherineSchulmanQuotesOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return CatherineSchulmanQuotesOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CatherineSchulmanQuotesOmni::structfix($now, Struct::undef()))
                        !== json_encode(CatherineSchulmanQuotesOmni::structfix($val, Struct::undef()))) {
