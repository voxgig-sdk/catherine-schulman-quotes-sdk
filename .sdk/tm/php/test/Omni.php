...
//    only - a typed CatherineSchulmanQuotesContext there would make every ctx
...
final class CatherineSchulmanQuotesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CatherineSchulmanQuotesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CatherineSchulmanQuotesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CatherineSchulmanQuotesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CatherineSchulmanQuotesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CatherineSchulmanQuotesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CatherineSchulmanQuotesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CatherineSchulmanQuotesError(is_string($code) ? $code : '', $msg);
    }
