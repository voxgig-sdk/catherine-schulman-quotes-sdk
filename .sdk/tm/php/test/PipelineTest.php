...
// CatherineSchulmanQuotes SDK pipeline test
//
...
{
    private static function utility(): CatherineSchulmanQuotesUtility
    {
        return new CatherineSchulmanQuotesUtility();
    }
    ...
    private static function ctx(array $over = []): CatherineSchulmanQuotesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CatherineSchulmanQuotesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CatherineSchulmanQuotesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CatherineSchulmanQuotesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CatherineSchulmanQuotesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CatherineSchulmanQuotesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CatherineSchulmanQuotesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CatherineSchulmanQuotesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CatherineSchulmanQuotesResponse([]),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        [, $err] = CatherineSchulmanQuotesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec([]),
            'response' => null,
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        [, $err] = CatherineSchulmanQuotesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's']),
            'response' => new CatherineSchulmanQuotesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        [, $err] = CatherineSchulmanQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's']),
            'response' => new CatherineSchulmanQuotesResponse(self::resp(200, ['v' => 1])),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        [, $err] = CatherineSchulmanQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's']),
            'response' => new CatherineSchulmanQuotesResponse(self::resp(200, ['v' => 2])),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CatherineSchulmanQuotesMakeResponse::call($ctx);
    ...
    {
        $preset = new CatherineSchulmanQuotesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec([]),
            'response' => new CatherineSchulmanQuotesResponse([]),
            'result' => new CatherineSchulmanQuotesResult([]),
        ...
        [$got, $err] = CatherineSchulmanQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CatherineSchulmanQuotesResult([])]);
        [, $err] = CatherineSchulmanQuotesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CatherineSchulmanQuotesSpec([]), 'result' => null]);
        [, $err] = CatherineSchulmanQuotesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's']),
            'result' => new CatherineSchulmanQuotesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CatherineSchulmanQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's']),
            'result' => new CatherineSchulmanQuotesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CatherineSchulmanQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CatherineSchulmanQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CatherineSchulmanQuotesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CatherineSchulmanQuotesSpec([]),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CatherineSchulmanQuotesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CatherineSchulmanQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CatherineSchulmanQuotesError('boom', 'boom');
        $utility->fetcher = function (CatherineSchulmanQuotesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CatherineSchulmanQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CatherineSchulmanQuotesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CatherineSchulmanQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CatherineSchulmanQuotesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CatherineSchulmanQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CatherineSchulmanQuotesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CatherineSchulmanQuotesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CatherineSchulmanQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CatherineSchulmanQuotesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CatherineSchulmanQuotesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CatherineSchulmanQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CatherineSchulmanQuotesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CatherineSchulmanQuotesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CatherineSchulmanQuotesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CatherineSchulmanQuotesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CatherineSchulmanQuotesResult(['ok' => false])]);
        $this->expectException(CatherineSchulmanQuotesError::class);
        CatherineSchulmanQuotesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CatherineSchulmanQuotesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CatherineSchulmanQuotesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CatherineSchulmanQuotesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CatherineSchulmanQuotesBaseFeature();
        $b = new CatherineSchulmanQuotesBaseFeature();
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $a);
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CatherineSchulmanQuotesBaseFeature
    {
        $f = new CatherineSchulmanQuotesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CatherineSchulmanQuotesFeatureAdd::call($ctx, self::named_feature('a'));
        CatherineSchulmanQuotesFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $replace);
        ...
        CatherineSchulmanQuotesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CatherineSchulmanQuotesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CatherineSchulmanQuotesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CatherineSchulmanQuotesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CatherineSchulmanQuotesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CatherineSchulmanQuotesResponse(['headers' => null]),
            'result' => new CatherineSchulmanQuotesResult([]),
        ]);
        CatherineSchulmanQuotesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CatherineSchulmanQuotesResultBody::call($ctx);
