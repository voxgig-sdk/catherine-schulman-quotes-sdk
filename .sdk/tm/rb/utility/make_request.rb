...
module CatherineSchulmanQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CatherineSchulmanQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = CatherineSchulmanQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CatherineSchulmanQuotesResponse.new(fetched)
    else
