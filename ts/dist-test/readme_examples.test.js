...
const __1 = require("..");
const SDK_NAME = 'CatherineSchulmanQuotesSDK';
...
const TEST_SEED = { "entity": { "episode": { "test01": { "id": "test01" } }, "episode_status": { "test01": { "id": "test01" } }, "quote": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CatherineSchulmanQuotesSDK\b/.test(code) ||
        /\bCatherineSchulmanQuotesSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CatherineSchulmanQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CatherineSchulmanQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
