...
require("catherine-schulman-quotes_types")
...
local CatherineSchulmanQuotesSDK = {}
CatherineSchulmanQuotesSDK.__index = CatherineSchulmanQuotesSDK
...
CatherineSchulmanQuotesSDK._make_feature = _make_feature


function CatherineSchulmanQuotesSDK.new(options)
  local self = setmetatable({}, CatherineSchulmanQuotesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CatherineSchulmanQuotesSDK:options_map()
...
function CatherineSchulmanQuotesSDK:get_utility()
  return Utility.copy(self._utility)
end


function CatherineSchulmanQuotesSDK:get_root_ctx()
  return self._rootctx
end


function CatherineSchulmanQuotesSDK:prepare(fetchargs)
...
function CatherineSchulmanQuotesSDK:direct(fetchargs)
...
function CatherineSchulmanQuotesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CatherineSchulmanQuotesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CatherineSchulmanQuotesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CatherineSchulmanQuotesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CatherineSchulmanQuotesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CatherineSchulmanQuotesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CatherineSchulmanQuotesSDK:Episode(data)
...
-- Idiomatic facade: client:EpisodeStatus():list() / client:EpisodeStatus():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CatherineSchulmanQuotesSDK:EpisodeStatus(data)
  local EntityMod = require("entity.episode_status_entity")
  if data == nil then
    if self._episode_status == nil then
      self._episode_status = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function CatherineSchulmanQuotesSDK:Quote(data)
...
function CatherineSchulmanQuotesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CatherineSchulmanQuotesSDK.new(sdkopts)
  sdk.mode = "test"
...
return CatherineSchulmanQuotesSDK
