# CatherineSchulmanQuotes Lua SDK Reference

Complete API reference for the CatherineSchulmanQuotes Lua SDK.
...
```lua
local episode_status = client:EpisodeStatus(nil)
```
...
```lua
local result, err = client:EpisodeStatus():load()
```
