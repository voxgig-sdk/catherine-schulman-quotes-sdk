# catherine-schulman-quotes-cli

boru-driven command-line client **and** interactive REPL for the CatherineSchulmanQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/catherine-schulman-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./catherine-schulman-quotes-cli --help

# 3. Provide credentials once, via the environment
export CATHERINE_SCHULMAN_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./catherine-schulman-quotes-cli list episode
./catherine-schulman-quotes-cli load 1 episode            # {id:1} shorthand
./catherine-schulman-quotes-cli load '{id:1}' episode       # explicit match map
./catherine-schulman-quotes-cli list episode_status

# 5. Override the API base URL for a single call
CATHERINE_SCHULMAN_QUOTES_BASE=https://api.example.com ./catherine-schulman-quotes-cli list episode

# 6. No arguments -> interactive REPL
./catherine-schulman-quotes-cli
catherine-schulman-quotes> list episode
catherine-schulman-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/catherine-schulman-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/catherine-schulman-quotes-cli list episode
   ```
...
```sh
./catherine-schulman-quotes-cli load 1 episode          # scalar shorthand for {id:1}
./catherine-schulman-quotes-cli load '{id:1}' episode     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `catherine-schulman-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./catherine-schulman-quotes-cli
catherine-schulman-quotes> list episode
catherine-schulman-quotes> /help
catherine-schulman-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/catherine-schulman-quotes-cli`. |
...
(`./catherine-schulman-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
