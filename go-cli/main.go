// Command CatherineSchulmanQuotes-cli is a CLI + REPL for the CatherineSchulmanQuotes SDK,
...
const prompt = "catherine-schulman-quotes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "episode episode_status quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CATHERINE_SCHULMAN_QUOTES_APIKEY (api key), CATHERINE_SCHULMAN_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CATHERINE_SCHULMAN_QUOTES_APIKEY carries the API key and
	// CATHERINE_SCHULMAN_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CATHERINE_SCHULMAN_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CATHERINE_SCHULMAN_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCatherineSchulmanQuotesSDK(opts)
