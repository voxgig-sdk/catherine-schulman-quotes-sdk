# catherine-schulman-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CatherineSchulmanQuotes SDK as
two agent tools — `catherine-schulman-quotes_list` and `catherine-schulman-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/catherine-schulman-quotes-mcp)
make build

# 2. Provide credentials via the environment
export CATHERINE_SCHULMAN_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user catherine-schulman-quotes \
  -- /absolute/path/to/catherine-schulman-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./catherine-schulman-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// catherine-schulman-quotes_list: first page of records
{ "entity": "episode" }
{ "entity": "episode", "query": { } }

// catherine-schulman-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/catherine-schulman-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user catherine-schulman-quotes \
     -- "$PWD"/dist/*/catherine-schulman-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `catherine-schulman-quotes_list` and `catherine-schulman-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list episode using catherine-schulman-quotes"*
   and it calls `catherine-schulman-quotes_list` with `{"entity":"episode"}`.
...
```sh
export CATHERINE_SCHULMAN_QUOTES_APIKEY=sk_live_xxx            # API key
export CATHERINE_SCHULMAN_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `catherine-schulman-quotes_list` tool
...
### Call the `catherine-schulman-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `catherine-schulman-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `catherine-schulman-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CATHERINE_SCHULMAN_QUOTES_APIKEY` | API key sent with every request. |
| `CATHERINE_SCHULMAN_QUOTES_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

episode | episode_status | quote

### Smoke test via HTTP (raw JSON-RPC)

```sh
./catherine-schulman-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"catherine-schulman-quotes_load","arguments":{"entity":"episode","query":{"id":1}}}}'
```
