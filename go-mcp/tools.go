...
type Args struct {
	Entity string         `json:"entity" jsonschema:"episode | episode_status | quote"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CatherineSchulmanQuotesSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "catherine-schulman-quotes_list",
		Description: "List records from CatherineSchulmanQuotes. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "catherine-schulman-quotes_load",
		Description: "Load a single record from CatherineSchulmanQuotes. " +
...
func runOp(client *sdk.CatherineSchulmanQuotesSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CatherineSchulmanQuotesSDK, name string) (sdk.CatherineSchulmanQuotesEntity, error) {
	switch strings.ToLower(name) {
	case "episode":
		return client.Episode(nil), nil
	case "episode_status":
		return client.EpisodeStatus(nil), nil
	case "quote":
