...
class EpisodeStatusEntity
  ...
  def make
    opts = @_entopts.dup
    EpisodeStatusEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CatherineSchulmanQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EpisodeStatus, Hash] the current EpisodeStatus data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CatherineSchulmanQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EpisodeStatus fields)
  def match_get
  ...
  #
  # @param reqmatch [EpisodeStatusLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.EpisodeStatus.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [EpisodeStatus, Hash] the loaded EpisodeStatus; raises CatherineSchulmanQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CatherineSchulmanQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
