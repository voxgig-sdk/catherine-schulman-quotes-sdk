...
module CatherineSchulmanQuotesUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = CatherineSchulmanQuotesContext.new({}, nil)
    end
    op = ctx.op || CatherineSchulmanQuotesOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || CatherineSchulmanQuotesResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(CatherineSchulmanQuotesError) ? err.msg : err.to_s
    ...
    sdk_err = CatherineSchulmanQuotesError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(CatherineSchulmanQuotesError)
