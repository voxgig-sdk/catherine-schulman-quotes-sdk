...
//
// NewCatherineSchulmanQuotesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.CatherineSchulmanQuotesSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewCatherineSchulmanQuotesSDK(opts)
}
...
func fcCandidates(client *sdk.CatherineSchulmanQuotesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.CatherineSchulmanQuotesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.CatherineSchulmanQuotesSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.CatherineSchulmanQuotesError
