...
import (
	...
	omni "github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CatherineSchulmanQuotesError); is {
				out := map[string]any{
					"name":    "CatherineSchulmanQuotesError",
...
func omniCtx(arg any, client *sdk.CatherineSchulmanQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CatherineSchulmanQuotesSDK); is {
			client = live
