# CatherineSchulmanQuotes Golang SDK



The Golang SDK for the CatherineSchulmanQuotes API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/catherine-schulman-quotes-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go=../catherine-schulman-quotes-sdk/go
```
...
import (
    "fmt"
    sdk "github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go"
)

func main() {
    client := sdk.New()

    // List episode records — the value is the array of records itself.
    episodes, err := client.Episode(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range episodes.([]any) {
        fmt.Println(item)
    }
...
```
CATHERINE_SCHULMAN_QUOTES_TEST_LIVE=TRUE
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CatherineSchulmanQuotesSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### CatherineSchulmanQuotesSDK methods
...
| `Episode` | `(data map[string]any) CatherineSchulmanQuotesEntity` | Create an Episode entity instance. |
| `EpisodeStatus` | `(data map[string]any) CatherineSchulmanQuotesEntity` | Create an EpisodeStatus entity instance. |
| `Quote` | `(data map[string]any) CatherineSchulmanQuotesEntity` | Create a Quote entity instance. |

### Entity interface (CatherineSchulmanQuotesEntity)

All entities implement the `CatherineSchulmanQuotesEntity` interface.
...
slice):
    ...
    episode, err := client.Episode(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
...
Create an instance: `episodeStatus := client.EpisodeStatus(nil)`
...
```go
episodeStatus, err := client.EpisodeStatus(nil).Load(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(episodeStatus) // the loaded record
```
...
```
github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go/
├── catherine-schulman-quotes.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/catherine-schulman-quotes-sdk/go`) re-exports everything needed
