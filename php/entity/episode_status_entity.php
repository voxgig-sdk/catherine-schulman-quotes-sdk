...
// CatherineSchulmanQuotes SDK EpisodeStatus entity
...
class EpisodeStatusEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EpisodeStatusEntity($this->_client, $opts);
    }

    /**
     * @param EpisodeStatus|array $args EpisodeStatus data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CatherineSchulmanQuotesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EpisodeStatus|array The current EpisodeStatus data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EpisodeStatus fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CatherineSchulmanQuotesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EpisodeStatus fields).
     */
    ...
    /**
     * Load a single EpisodeStatus.
     *
     * @param EpisodeStatusLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EpisodeStatusLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EpisodeStatus|array The loaded EpisodeStatus as an assoc-array at the
     *   SDK boundary; throws CatherineSchulmanQuotesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CatherineSchulmanQuotesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
