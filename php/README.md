# CatherineSchulmanQuotes PHP SDK



The PHP SDK for the CatherineSchulmanQuotes API — an entity-oriented client using PHP conventions.
...
```php
<?php
require_once 'catherineschulmanquotes_sdk.php';

$client = new CatherineSchulmanQuotesSDK();
```
...
```php
try {
    // list() returns an array of Episode records — iterate directly.
...
```php
require_once 'catherineschulmanquotes_sdk.php';
$client = new CatherineSchulmanQuotesSDK($options);
```
...
```php
$client = CatherineSchulmanQuotesSDK::test($testopts, $sdkopts);
```
...
Create an instance: `$episode_status = $client->EpisodeStatus();`
...
```
php/
├── catherineschulmanquotes_sdk.php          -- Main SDK class
...
The main class (`catherineschulmanquotes_sdk.php`) exports the SDK class
