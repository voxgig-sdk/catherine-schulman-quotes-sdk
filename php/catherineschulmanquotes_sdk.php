...
class CatherineSchulmanQuotesSDK
{
    public string $mode;
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new CatherineSchulmanQuotesUtility();
        $this->_utility = $utility;

        $config = CatherineSchulmanQuotesConfig::shared_config();
        ...
        $feature_opts = CatherineSchulmanQuotesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CatherineSchulmanQuotesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CatherineSchulmanQuotesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CatherineSchulmanQuotesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CatherineSchulmanQuotesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CatherineSchulmanQuotesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CatherineSchulmanQuotesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CatherineSchulmanQuotesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CatherineSchulmanQuotesError($op . "_allow",
                "CatherineSchulmanQuotesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CatherineSchulmanQuotesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CatherineSchulmanQuotesError("graphql_error",
                "CatherineSchulmanQuotesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_episode_status = null;

    // Canonical facade: $client->EpisodeStatus()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->episode_status()
    // resolves here too.
    public function EpisodeStatus($data = null)
    {
        require_once __DIR__ . '/entity/episode_status_entity.php';
        if ($data === null) {
            if ($this->_episode_status === null) {
                $this->_episode_status = new EpisodeStatusEntity($this, null);
            }
            return $this->_episode_status;
        }
        return new EpisodeStatusEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CatherineSchulmanQuotesSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
