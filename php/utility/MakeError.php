...
// CatherineSchulmanQuotes SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class CatherineSchulmanQuotesMakeError
{
    public static function call(?CatherineSchulmanQuotesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CatherineSchulmanQuotesContext([], null);
        }
        $op = $ctx->op ?? new CatherineSchulmanQuotesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CatherineSchulmanQuotesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CatherineSchulmanQuotesError) ? $err->msg : (string)$err;
        $msg = "CatherineSchulmanQuotesSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new CatherineSchulmanQuotesError('', $msg, $ctx);
        ...
        if ($err instanceof CatherineSchulmanQuotesError) {
