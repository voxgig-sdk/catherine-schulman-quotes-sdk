...
// CatherineSchulmanQuotes SDK utility: result_basic

class CatherineSchulmanQuotesResultBasic
{
    public static function call(CatherineSchulmanQuotesContext $ctx): ?CatherineSchulmanQuotesResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof CatherineSchulmanQuotesError) ? $result->err->msg : (string)$result->err;
