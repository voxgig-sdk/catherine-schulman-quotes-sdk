...
// CatherineSchulmanQuotes SDK retry feature
...
class CatherineSchulmanQuotesRetryFeature extends CatherineSchulmanQuotesBaseFeature
{
    ...
    public function init(CatherineSchulmanQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CatherineSchulmanQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CatherineSchulmanQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CatherineSchulmanQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
