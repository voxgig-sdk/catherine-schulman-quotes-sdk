...
// CatherineSchulmanQuotes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CatherineSchulmanQuotesTestFeature extends CatherineSchulmanQuotesBaseFeature
{
    ...
    public function init(CatherineSchulmanQuotesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CatherineSchulmanQuotesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CatherineSchulmanQuotesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CatherineSchulmanQuotesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CatherineSchulmanQuotesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CatherineSchulmanQuotesParam::call($ctx, $k);
