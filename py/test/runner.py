# CatherineSchulmanQuotes SDK test runner
...
class CatherineSchulmanQuotesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CatherineSchulmanQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CatherineSchulmanQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CatherineSchulmanQuotesTestRunner.getenv("CATHERINE_SCHULMAN_QUOTES_TEST_LIVE")
        override = CatherineSchulmanQuotesTestRunner.getenv("CATHERINE_SCHULMAN_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CatherineSchulmanQuotesTestRunner.getenv(key)
        ...
        explain = CatherineSchulmanQuotesTestRunner.getenv("CATHERINE_SCHULMAN_QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CATHERINE_SCHULMAN_QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CatherineSchulmanQuotesTestRunner._test_control is not None:
            return CatherineSchulmanQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CatherineSchulmanQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CatherineSchulmanQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CatherineSchulmanQuotesTestRunner.load_test_control()
...
def load_env_local():
    CatherineSchulmanQuotesTestRunner.load_env_local()


def env_override(m):
    return CatherineSchulmanQuotesTestRunner.env_override(m)


def entity_data(v):
    return CatherineSchulmanQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CatherineSchulmanQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CatherineSchulmanQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CatherineSchulmanQuotesTestRunner.load_test_control()


def live_client_options():
    return CatherineSchulmanQuotesTestRunner.live_client_options()


def live_delay_ms():
    return CatherineSchulmanQuotesTestRunner.live_delay_ms()
