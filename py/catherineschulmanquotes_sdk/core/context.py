...
from catherineschulmanquotes_sdk.core.helpers import get_ctx_prop, to_map
...
class CatherineSchulmanQuotesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = CatherineSchulmanQuotesControl()
        ...
        if isinstance(sp, CatherineSchulmanQuotesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CatherineSchulmanQuotesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CatherineSchulmanQuotesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return CatherineSchulmanQuotesOperation({})
        ...
        op = CatherineSchulmanQuotesOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return CatherineSchulmanQuotesError(code, msg, self)
